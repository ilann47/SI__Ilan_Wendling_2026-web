...
Os catálogos base foram isolados pelas V50/V51 e Produto e Serviço pela V52. As
cinco telas usam JWT contextual, `catalog:read` para leitura e `catalog:manage`
para criação, alteração e exclusão. O tenant é resolvido no servidor e não
integra URL, query string nem payload.
...
`/api/produtos` e `/api/servicos`. GET exige `catalog:read`; POST, PUT e DELETE
exigem `catalog:manage`.
...
ficam fora do payload; `false`, `0` e `null` válidos são preservados. Tenant,
versão e estado não são enviados como campos editáveis.
...
Os cinco CRUDs habilitam locking otimista explicitamente. Detalhe, criação e
...
- ProdutoFornecedor, notas fiscais, relatórios e estoque não foram ativados.
- Consumers históricos continuam globais e não podem selecionar produtos ou
  serviços tenant-aware até seus próprios contratos receberem isolamento.
